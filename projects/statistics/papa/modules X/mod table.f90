...
!           subroutines
            ...
            procedure, public :: initialize_solvent_players  =>  initialize_solvent_players_sub
...
!       ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                  play_one_round
        ...
        subroutine play_one_round_sub ( self )
            ...
            !character ( len = * ),parameter            :: me_subroutine = 'subroutine play_one_round_sub'  ! self-identification

                call self % place_bets ( )
                call self % deal_hands ( )  ! catches blackjacks
                self % pointerBustList => self % pLBust
                self % pointerOpenList => self % pLOpen
                !print *, me_module, ', ', me_subroutine, ': size of bust list = ', size ( self % pointerBustList )
                !print *, me_module, ', ', me_subroutine, ': size of open list = ', size ( self % pointerOpenList )
                    call self % sweep_players ( self % pointerBustList, self % pointerOpenList, &
                                                self % numPHandsBust,   self % numPHandsOpen, &
                                                self % show_card )
                self % pointerBustList => null ( )
                self % pointerOpenList => null ( )

        end subroutine play_one_round_sub

!       ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                   sweep_players

        subroutine sweep_players_sub ( self, BustList, OpenList, nBust, nOpen, show_card )
            ...
            !character ( len = * ), parameter           :: me_subroutine = 'subroutine sweep_players_sub'  ! self-identification
...
!               sweep solvent players
                pShoe => self % myShoe
                    do kTable = 1, self % numPlayersSolvent
                        ...
                        !write ( *, '( "kTable = ", g0, ", playerID = ", g0 )' ) kTable, playerID
                            player % numPlayerHands = 1
                            !write ( *, '( g0, 2X, g0, 2X, "handID = ", g0 )' ) me_module, &
                            !                                                  me_subroutine, &
                            !                                                  player % myHands ( 1 ) % handID
                            !write ( *, '( g0, 2X, g0, 2X, "best_value = ", g0 )' ) me_module, &
                            !                                                      me_subroutine, &
                            !                                                      player % myHands ( 1 ) % best_value
                            call player % play_my_hands ( pSHoe, BustList, OpenList, nBust, nOpen, show_card )
                            player => null ( )
                        end do
                pShoe => null ( )
...
!               deal cards
                shoe => self % myShoe
                    ...
                    self % show_card = self % dealer % myHands ( 1 ) % cards ( dealerShow )
                    write ( *, 100 ) self % show_card
...
!               check dealer for blackjack
                ...
                return
    100         format ( 'Dealer show card = ', g0 )
...
!               print *, me_subroutine, ': size of bust list = ', size ( self % pLBust )
...
!       ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++      initialize_solvent_players

        subroutine initialize_solvent_players_sub ( self )
        ...
        end subroutine initialize_solvent_players_sub
...
!       ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                  who_is_solvent
        ...
        subroutine who_is_solvent_sub ( self )
            ...
            type ( banks ), pointer  :: playerBank
                ...
                do kTable = 1, self % numPlayersTotal
                    playerBank => self % myPlayers ( kTable ) % myBank
                    delta = playerBank % now - chip
                    if ( delta >= epsilon ( one ) ) then
                        jTable = jTable + 1
                        write ( *, 100 ) jTable, playerBank % now
                        self % lPlayersSolvent ( jTable ) = kTable
                    end if
                    playerBank => null ( )
                end do

                self % numPlayersSolvent = jTable
                if ( jTable == 0 ) self % fGameOver = .true.

            return

    100     format ( 'Player ', g0, ' balance = ', g0 )
