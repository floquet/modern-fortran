...
!                allocate ( cards ( 1 : nUnique**numCards, 1 : numCards ), stat = alloc_status, errmsg = alloc_msg )
!                allocate ( cards ( 1 : 100, 1 : 2 ), stat = alloc_status, errmsg = alloc_msg )
                ...
                write ( *, '( "output file = data/", g0 )' ) file_name
                write ( *, '( "bytes needed to store cards = ", g0 )' ) sizeof ( cards )
                write ( *, '( "size ( cards, 1 ) = ", g0, "; size ( cards, 2 ) = ", g0 )') size ( cards, 1 ), size ( cards, 2 )
                write ( *, '( "number of rows = ", g0, "; number of columns = ", g0 )' ) nUnique**numCards, nUnique

                open ( newunit = io, file = 'data/' // file_name, action = 'write', status = 'new', form = 'unformatted', &
