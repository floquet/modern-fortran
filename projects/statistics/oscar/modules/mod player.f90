...
!       ############################################################################################
        ...
        subroutine play_my_hands_sub ( self, pSHoe, BustList, OpenList, nBust, nOpen )
...
!               loop over hands
                ...
                do kPlayer = 1, maxHandsPP
                    call self % myHands ( kPlayer ) % play_this_hand ( self % playerID, self % rule, pSHoe, &
                                                                       BustList, OpenList, nBust, nOpen )
                    if ( kPlayer >= self % numPlayerHands ) return
                end do
