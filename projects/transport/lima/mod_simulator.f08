...
module mSimulator
    ...
    use mCapsules,              only : capsules
    use mPopulations,           only : populations
    use mSimulationParameters,  only : numDims, nMaxCapsules
    use mUtilities,             only : export_generic_anyrank_sub

    implicit none

    integer, private             :: jSim = 0, kSim = 0 ! dummy counter for this module
...
!                                                                      mfp = [ one, 0.01_wp, 3.0_wp, 10.0_wp, one ] )
                ...
                if ( present ( fPrintLoad ) ) then ! print capsule structure
                    if ( fPrintLoad ) then
                ...
                ! populate capsules
                do kSim = 1, me % numCapsules
                    call me % myCapsules ( kSim ) % load_capsule_population ( kSim * 10000000 )
                    call me % myCapsules ( kSim ) % determine_radial_zone ( )
                end do

                if ( present ( fPrintPopulate ) ) then ! print population data to screen
!                     jSim = 1
