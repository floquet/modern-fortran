...
!*
!*    This program is the inviscid (implicit) version: OSSAN-Euler2D-Implicit
!*
...
!*
!*  (2) jacobian.f90        : Construct a linear system, [V/dt+dR/dU]*dU = -Residual
!*  (3) linear_solve.f90    : Solve the linear system (GS relaxation)
!*  (4) euler_solver.f90    : Integrate the Euler equations to steady state
!*  (5) main.f90            : Main driver code
!*
!* ------------------------------------------------------------------------------
!* Notes:
!*
...
!********************************************************************************
 ...
 use my_main_data, only : M_inf, gamma, iteration_method,             &
                          CFLexp, CFL1, CFL2, CFL_ramp_steps, sweeps, &
                          tolerance, max_iterations,                  &
                          inviscid_flux, inviscid_jac,                &
                          jac, nnodes, node

 use euler_solver, only : euler_solver_main ! Steady Euler solver
...
!Local variables
 integer       :: i
  ...
  write(*,*) "***************************************************************"
  write(*,*) " Starting OSSAN-Euler2D-Implicit "
  write(*,*) "***************************************************************"
...
!  For explicit scheme(RK2)

            CFLexp = 0.99_p2    ! CFL for expicit method (RK2); must be small

!  For implicit scheme(DC with a pseudo time term)
...
!        E.g., Increase 'sweeps' to solve the linear system better.
              ...
              CFL2 = 1.0e+14_p2 !   Final CFL for implicit method
    CFL_ramp_steps = 20         ! Number of iterations to reach from CFL1 to CFL2
            sweeps = 15         ! Number of linear GS sweeps for implicit method

         tolerance = 1.0e-15_p2 ! Residual tolerance for steady computations
    max_iterations = 500000     ! Max number of iterations
...
!     For implicit method, allocate the array that stores a Jacobian matrix
      if (trim(iteration_method) == "implicit") then
       allocate(jac(nnodes))
       do i = 1, nnodes
         allocate( jac(i)%diag(4,4)                 ) ! Diagonal block
         allocate( jac(i)%off(4,4, node(i)%nnghbrs) ) ! Off-diagonal block
       end do
      endif
...
! (4) Set initial solution
      call set_initial_solution

! (5) Compute the numerical solution
...
!*         Only two BCs are available in this version:
!*
!*         1. "freestream"
!*             Roe flux with free stream condition on the right state.
!*
!*         2. "slip_wall_weak"
!*             Solid wall condition via the pressure flux (weak condition)
!*
!********************************************************************************
