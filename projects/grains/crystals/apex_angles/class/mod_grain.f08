...
module mGrain
    ...
    use mConstants,            only : pi, rad_to_deg
    ...
    integer ( ip ), private :: kGrain

    type :: grain
        type ( axis )  :: axes        ( 1 : 3 )
        type ( angle ) :: apex_angle  ( 1 : 3 )
        type ( angle ) :: total_angle
...
contains
    ...
    subroutine print_apex_angles_sub ( me )
        ...
        class ( grain ), target :: me
            ...
            write ( * , '( /, "Apex angles (ideal answer = pi/3 ≈ 1.04719755 or 60 deg):" )' )
            ...
            write ( * , '( /, "Total of all angles (ideal answer = pi or 180 deg ):" )' )
            write ( * , 110 ) me % total_angle % th,              me % total_angle % dth, &
                              me % total_angle % th * rad_to_deg, me % total_angle % dth * rad_to_deg

            return

        100 format ( I3, 2X, F10.5, ' +/- ', F10.5, 5X, '(', F10.5, ' +/- ', F10.5,' ) degrees' )
        110 format ( F10.5, ' +/- ', F10.5, 5X, '(', F10.5, ' +/- ', F10.5,' ) degrees' )
    ...
    subroutine compute_angles_sub ( me )
        ...
        class ( grain ), target :: me
            ...
            ! total of three angles should be pi
            me % total_angle %  th = me % apex_angle ( 1 ) % th + me % apex_angle ( 2 ) % th + me % apex_angle ( 3 ) % th

            me % total_angle % dth = sqrt ( me % apex_angle ( 1 ) % dth ** 2 + &
                                            me % apex_angle ( 2 ) % dth ** 2 + &
                                            me % apex_angle ( 3 ) % dth ** 2 )
