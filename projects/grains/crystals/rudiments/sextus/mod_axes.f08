...
!       ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++             solve_linear_system
    ...
    subroutine solve_linear_system_sub ( me )
        ...
        real ( rp ) :: det = zero, SSE = zero
            ...
            call open_file_print_matrix ( Winv, 'E10.3', 2, 'Winv', myFile = trim ( me % path_case ) // 'Winv.txt' )
            ...
            call open_file_print_matrix ( me % matrix_A, 'F10.3', 2, 'System matrix A', &
...
!   ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                     export_solution
    ...
    subroutine export_solution_sub ( me )
        ...
        character ( len = * ), parameter       :: stop_msg  = 'Execution ending due to error in ' // me_mAxes // &
            ...
            ! write file
            write  ( my_io_unit, '( "data set = ", g0, "." )', iostat = io_stat, iomsg = io_msg ) trim( me % iam )
            write  ( my_io_unit, fmt_results, iostat = io_stat, iomsg = io_msg ) 'intercept', me % results % intercept, &
                                                                                              me % results % err_intercept
            write  ( my_io_unit, fmt_results, iostat = io_stat, iomsg = io_msg ) 'gap      ', me % results % slope, &
                                                                                              me % results % err_slope
            write  ( my_io_unit, fmt_results, iostat = io_stat, iomsg = io_msg ) 'slope    ', me % results % gap, &
                                                                                              me % results % err_gap
...
!   ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++             populate_column_vectors
    ...
    subroutine populate_column_vectors_sub ( me, xyphi )
        ...
        character ( len = 512 )                :: myFile = ''
            ...
            call open_file_output ( file_name = myFile, io_unit = io_debug )

            start = 1
            do kAxis = 1, me % nParts
                end = start + me % partition ( kAxis ) - 1
                write ( io_debug , '( "Partition ", g0, " has ", g0, " members (elements ", g0, " to ", g0, "):" )' ) &
                        kAxis, me % partition ( kAxis ), start, end
                write ( io_debug , * ) me % members ( start : end )
            ...
            close ( unit = io_debug, iostat = io_stat, iomsg = io_msg )
            if ( io_stat /= 0 ) then
                write ( * , fmt = fmt_ioerror ) 'CLOSE', trim ( myFile ), io_debug
                write ( * , fmt = fmt_iostat  ) io_stat
                write ( * , fmt = fmt_iomsg   ) trim ( io_msg )
                write ( * , fmt = fmt_iocont  )
            end if
...
!   ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                        read_members
    ...
    subroutine read_members_sub ( me )
        ...
        character ( len = 512 )                :: myFile = ''
            ...
            call open_file_output ( file_name = myFile, io_unit = io_debug )
            write ( io_debug , fmt = fmt_read_array ) me % nPoints, trim ( myFile )
            write ( unit = fmt_parts, fmt = fmt_x ) me % nPoints
            if ( io_stat /= 0 ) then
                write ( * , fmt = fmt_ioerror    ) 'WRITE', 'fmt_parts', io_debug
                write ( * , fmt = fmt_iostat     ) io_stat
                write ( * , fmt = fmt_iomsg      ) trim ( io_msg )
                stop stop_msg
            end if
            ...
            do kAxis = 1, me % nParts
                ...
                write ( io_debug , fmt = fmt_parts ) kAxis, ( ( me % members ( jAxis ) ), jAxis = start, end )
                start = end + 1
            end do
...
!   ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                      read_partition
    ...
    subroutine read_partition_sub ( me )
        ...
        character ( len = 512 )                :: myFile = ''
            ...
            call open_file_output ( file_name = myFile, io_unit = io_debug )
            write ( io_debug , fmt = fmt_read_array ) me % nParts, trim ( myFile )
