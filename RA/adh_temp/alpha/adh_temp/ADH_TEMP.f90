...
PROGRAM ADH_TEMP
  ...
  open( newunit = TEMP_CON, file = TEMP_CONFN, status = 'OLD', iostat = i, errmsg = io_msg )
  if (i /= 0) then
    !write(*,*) 'Control file does not exist.'
    write(*,'("Error opening file ",g0,/,"iostat = ",g0,", errmsg = ",g0,".")') TEMP_CONFN, i, trim(io_msg)
    stop
  end if
  !
  ! load DLL
  ...
  open( newunit = TEMP_OPT, file = TEMP_OPTFN, status = 'unknown')
