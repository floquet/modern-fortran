...
PROGRAM ADH_TEMP
  ...
  integer       :: na
  integer       :: groupIndex
  character*64  :: paramName, groupName
  character*256 :: errorMessage
  !
  real(8),      allocatable, dimension(:)  :: a
  character*64, allocatable, dimension(:)  :: names
  real(8),      allocatable, dimension(:)  :: paramReal, paramReal_theta
  integer,      allocatable, dimension(:)  :: paramInteger
  logical,      allocatable, dimension(:)  :: transport_a
  !
  real(8)      :: t, dt, tStart, tEnd
  integer      :: TEMP_CON, TEMP_OPT
  !
  ! Example inputs and outputs
  character*64, PARAMETER      :: TEMP_CONFN = 'ADH_TEMP_CON.npt'
  character*64, PARAMETER      :: TEMP_OPTFN = 'ADH_TEMP_RESULT.opt'
  ...
  character*64 :: name_tmpt
  character*8  :: switch, buffer
  logical      :: isTC
  ...
  open(TEMP_CON, file = TEMP_CONFN, status = 'OLD', iostat = i)
  if (i /= 0) then
    write(*,*) 'Controle file does not exist.'
    stop
  end if
  !
  ! load DLL
  ...
  open(TEMP_OPT, file = TEMP_OPTFN, status = 'unknown')
  ...
  !
  tStart = 0.0
  dt     = 5.0 * 60.0 / 86400.0         ! time step = 5 min
  tEnd   = 100 * dt 
  t      = tStart
  !
  do while (t <= tEnd)
