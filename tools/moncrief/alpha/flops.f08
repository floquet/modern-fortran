program flops
    ...
    external :: MPI_COMM_RANK, MPI_COMM_SIZE, MPI_INIT, MPI_FINALIZE

    ! parameters
    integer,        parameter :: ip = INT64, rp = REAL64
    integer ( ip ), parameter :: iterations = 10_ip ** 6, measures = 10_ip  ! check format descriptor 110 for measures
    ...
    real ( rp ) :: nu_ave = zero, nu_var = zero, nu_max = zero, nu_min = zero, alpha = zero
        ...
        ! write summary data
        file_results = 'summary' // '_' // trim ( path_results ) // trim ( machine_name ) // '_' // trim ( descriptor_test ) // &
        ...
        file_results = 'rawdata' // '_' // trim ( path_results ) // trim ( machine_name ) // '_' // trim ( descriptor_test ) // &
                       '_' // trim ( rack ) // '_'  // trim ( icu ) // '_'  // trim ( node ) // '_' // trim ( str_rank ) // '.csv'
