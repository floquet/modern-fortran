...
contains
    ...
    subroutine matmul_test ( io_handle, rank, hpc, rack )
        ...
        integer ( ip ) :: lambda = 20
            ...
            write ( io_handle, 100 ) trim ( hpc ), trim ( rack ), rank, 'matmul', 'cpu',         [ ( stats ( k ), k = 1, 5 ) ]

            stats = averager ( array_wall )
            write ( io_handle, 100 ) trim ( hpc ), trim ( rack ), rank, 'matmul', 'wall',        [ ( stats ( k ), k = 1, 5 ) ]

            stats = averager ( array_util )
            write ( io_handle, 100 ) trim ( hpc ), trim ( rack ), rank, 'matmul', 'utilization', [ ( stats ( k ), k = 1, 5 ) ]
            flush ( io_handle )
    ...
    subroutine tangent_test ( io_handle, rank, hpc, rack )
        ...
        integer ( ip ) :: lambda = 2**3!0
            ...
            stats = averager ( array_cpu )
            write ( io_handle, 100 ) trim ( hpc ), trim ( rack ), rank, 'tan-atan', 'cpu',         [ ( stats ( k ), k = 1, 5 ) ]

            stats = averager ( array_wall )
            write ( io_handle, 100 ) trim ( hpc ), trim ( rack ), rank, 'tan-atan', 'wall',        [ ( stats ( k ), k = 1, 5 ) ]

            stats = averager ( array_util )
            write ( io_handle, 100 ) trim ( hpc ), trim ( rack ), rank, 'tan-atan', 'utilization', [ ( stats ( k ), k = 1, 5 ) ]
            flush ( io_handle )
