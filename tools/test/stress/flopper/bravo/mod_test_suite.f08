...
contains
    ...
    subroutine scalar_tangent_test_bundler ( nu_ave, sigma_nu, nu_max, nu_min, alpha, &
                                             ...
                                             iterations, measures, time_sequence )
        real ( rp ),    intent ( out ) :: time_sequence ( : )
        ...
        character ( len =  10 ) :: time
            ...
            do k_measure = 1, measures  ! repeat measurements to resolve clearer picture of performance
                ...
                ! bound variation
                if ( 1.0_rp / time_cpu > nu_max ) then
                    nu_max = 1.0_rp / time_cpu
                else if ( 1.0_rp / time_cpu < nu_min ) then
                    nu_min = 1.0_rp / time_cpu
                end if

                time_sequence ( k_measure ) = 1.0_rp / time_cpu

            end do

            ! average and variance; parentheses for emphasis
            nu_ave    = real ( iterations, rp ) / real ( measures, rp ) * sum ( time_sequence )
            nu_sq_ave = real ( iterations, rp ) / real ( measures, rp ) * sum ( time_sequence * time_sequence )

            write ( * , 300 ) nu_ave, nu_sq_ave
        ...
        300 format ( 'nu_ave = ', g0, ', nu_sq_ave = ', g0 )
