program flops
    ...
    integer ( ip ), parameter :: iterations = 10_ip ** 9, measures = 20_ip  ! check format descriptor 110 for measures
    ...
    real ( rp ) :: nu_ave = zero, nu_var = zero, nu_max = zero, nu_min = zero, alpha = zero, time_cpu_total = zero
        ...
        call scalar_tangent_test_bundler ( nu_ave = nu_ave, nu_var = nu_var, nu_max = nu_max, nu_min = nu_min,      & 
                                           time_cpu_total = time_cpu_total, alpha = alpha, iterations = iterations, &
                                           measures = measures, frequency_sequence = frequency_sequence )
        ! dynamic data
        ...
        write ( io_out_results, 100 ) trim ( machine_name ), trim ( rack ), trim ( icu ), trim ( node ), rank,   & ! 5
                                      ...
                                      date, trim ( descriptor_test ), iterations, measures, nu_ave, nu_var, nu_max, nu_min, alpha ! 9
