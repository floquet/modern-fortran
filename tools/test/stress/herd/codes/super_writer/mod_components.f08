...
contains
    ...
    subroutine pbs_script ( account, queue, proc, cores, walltime, machine, nJobs, launch )
        ...
        ! write pbs files
        !call execute_command_line ( 'mkdir '   // machine // '/' // trim ( proc ) // '/' )
        ! status_chdir = getcwd ( cwd )
        ! print *, 'pbs current working directory = ', trim ( cwd ), '; status = ', status_chdir
        do kJobs = 1, nJobs
            write ( unit = who, fmt = '( I3.3 )' ) kJobs
            call execute_command_line ( 'mkdir ' // who )
            !print *, 'mkdir leo/gpu/' // who
            io_out = safeopen_writereplace ( trim ( cwd ) // '/' // who // '/' // machine // '-' &
                                                            // trim ( proc ) // '-' // who )
            ! io_out = safeopen_writereplace ( machine // '/' // trim ( proc ) // '/' // who // '/' // machine // '-' &
            !                                                 // trim ( proc ) // '-' // who )
    ...
    subroutine launcher_script ( machine, proc, nJobs )
        ...
        character ( len = * ), intent ( in ) :: machine, proc
            ...
            !io_out = safeopen_writereplace ( machine // '/' // trim ( proc ) // '/launcher' )
            do kJobs = 1, nJobs
                write ( unit = who, fmt = '( I3.3 )' ) kJobs
