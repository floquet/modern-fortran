...
contains
    ...
    subroutine pbs_script ( account, queue, cores, walltime, machine, nJobs )
        ...
        ! print *, 'mkdir gpu/'
        ! call execute_command_line ( 'cd gpu/' )
        ! print *, 'cd gpu/'
        !call execute_command_line ( 'pwd' )
        ...
        do kJobs = 1, nJobs
            ...
            io_out = safeopen_writereplace ( machine // '/' // proc // '/' // who // '/' // machine // '_' // who )
            ...
            write ( io_out, 120 )
            write ( io_out, 130 ) '$dir_mersenne/' // proc // '/'
    ...
    subroutine launcher_script ( machine, nJobs )
        ...
        character ( len = * ), intent ( in ) :: machine
            ...
            do kJobs = 1, nJobs
                write ( io_out, 300 ) proc
                write ( unit = who, fmt = '( I3.3 )' ) kJobs
                write ( io_out, 310 ) who
                write ( io_out, 320 ) machine, who
                write ( io_out, '( "" )' )
            end do
                write ( io_out, 300 )
                write ( io_out, '( "" )' )
        ...
        300 format ( 'cd $dir_mersenne/', g0, '/' )
        310 format ( 'cd ', g0, '/' )
        320 format ( 'qsub ', g0, '_', g0 )
