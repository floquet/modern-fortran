program stress
    ...
    external :: MPI_INIT, MPI_COMM_RANK, MPI_COMM_SIZE, MPI_FINALIZE
        ...
        io_results = safeopen_writereplace ( trim ( myGit ) // 'transporter/' // trim ( yo ) &
                                                            // "_" // trim ( str_rank ) // "_stress.txt" )
        ...
        info = 'cp /proc/cpuinfo ' // trim ( myPath ) // 'cpu.txt'
        call execute_command_line ( trim ( info ) )
        info = 'cp /proc/cpuinfo ' // trim ( myPath ) // 'mem.txt'
        call execute_command_line ( trim ( info ) )
