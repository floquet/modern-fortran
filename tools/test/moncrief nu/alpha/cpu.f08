program stress
    ...
    external :: MPI_INIT, MPI_COMM_RANK, MPI_COMM_SIZE, MPI_FINALIZE
        ...
        yo = trim ( machine_name ) // "_" // trim ( host_name ) // "_"
        myPath = trim ( myGit ) // 'transporter/'
        if ( rank == 0 ) call execute_command_line ( 'mkdir ' // trim ( myPath ) )
        ...
        io_results = safeopen_writereplace ( trim ( myPath ) // trim ( yo ) // trim ( str_rank ) // "_stress.txt" )
        write ( io_results, '( g0, " processors in use" )' ) numPE
        flush ( io_results )

        myPath = trim ( myPath ) // 'info/'
        if ( rank == 0 ) call execute_command_line ( 'mkdir ' // trim ( myPath) )
        info = 'cp /proc/cpuinfo ' // trim ( myPath ) // trim ( yo ) // trim ( str_rank ) // '_cpu.txt'
        call execute_command_line ( trim ( info ) )
        info = 'cp /proc/meminfo ' // trim ( myPath ) // trim ( yo ) // trim ( str_rank ) // '_mem.txt'
        call execute_command_line ( trim ( info ) )
