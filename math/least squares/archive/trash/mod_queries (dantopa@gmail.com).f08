...
module mQueries
    ...
    integer, private                   :: kQ
    integer ( ip ), parameter, private :: numArgsMax  = 50
...
!       ============================================================================================= harvest_command_line_arguments
        ...
        subroutine harvest_command_line_arguments ( echo )
            ...
            integer                          :: c_arg_length = 0, c_arg_status = 0

            character ( len =  16 )          :: c_arg_char = ''
            character ( len = 256 )          :: io_msg = ''

                numArgsInput = command_argument_count ( )

                do kQ = 1, numArgsMax  ! harvest command line arguments
                    call get_command_argument ( kQ, c_arg_char, c_arg_length, c_arg_status )
                ...
                if ( present ( echo ) ) then
                    if ( echo .eqv. .true. ) then
                        ...
                        write ( * , 210 ) min ( numArgsInput, numArgsMax )
                        write ( * , 220 ) c_arg_list ( 1 : numArgsInput )
                    end if
                end if
            ...
            210 format ( g0, ' command line arguments returned:' )
            220 format ( 50I10 )
