program stepper
    ...
    character ( len = * ), parameter :: myProgram = 'program stepper' ! self-identification

    ! variables
    real ( rp ), allocatable :: array ( : )
    real                     :: t_clock
    integer ( INT64 )        :: myIO, iostat, stat, j, k, status
    integer ( INT64 )        :: clock_count_start, clock_count_stop, clock_count_delta, &
                                clock_count_rate, clock_count_max
    character ( len = 512 )  :: iomsg, errmsg, hostnm
        ...
        file_name = prec_type // '_', //, trim ( hostnm ), //, '.txt'
        ...
        do k = 1, 1024
            ...
            write ( myIO, '( g0, ", ", g0 )' ) j * sizeof ( 1.0_rp ), t_clock
            flush ( myIO )
        end do
