...
program allocation_times
    ...
    ! rank 0
    real ( rp )                                 :: time_mean = 0.0_rp, time_var = 0.0_rp
    ...
    character ( len = 512 ) :: file_name = '', mydir = '', myfile = '', &
                               machine_name = '', pbs_jobid = '', pbs_jobname = '', wdhpc = '', host = '', run_cmd = '', &
                               errmsg = '', io_msg = ''
    ...
    character ( len =   8 ) :: myDate = ''
        ...
        call get_environment_variable ( "wdhpc", wdhpc ) ! e.g. topaz
        ...
        myfile = trim ( machine_name ) // '_' // trim ( host ) // '_' // prec_type // '_' // myDate // '.txt'
        ...
        write ( *, '( "data file: ", g0 ) ' )     trim ( myfile )
        write ( *, '( "data directory: ", g0, / )' ) trim ( mydir )
        ...
        write ( myIO, '( "Fortran compiler version: ", g0 )'    ) compiler_version ( )
        write ( myIO, '( "Fortran compiler options: ", g0, / )' ) compiler_options ( )

        write ( myIO, '( "Machine: ", g0, ", node: ", g0 )' ) trim ( machine_name ), trim ( host )
        write ( myIO, '( "PBS job ID: ", g0 )' ) trim ( pbs_jobid )
        write ( myIO, '( "PBS job name: ", g0 )' ) trim ( pbs_jobname )
        write ( myIO, 300 ) myDate ( 1 : 4 ), myDate ( 5 : 6 ), myDate ( 7 : 8 ), myTime ( 1 : 2 ), myTime ( 3 : 4 ), myTime ( 5 :)
        write ( myIO, 310 ) trim ( run_cmd )

        write ( myIO, '( "Number of times measurement is repeated: ", g0 )' ) measurements
        ...
        do k = 1, numElements ! loop over sample sizes
            ...
            do j = 1, measurements ! repeat measurement
                call system_clock ( clock_count_start )
                    ...
                    if ( stat /= 0 ) then
                        ...
                        write ( myIO, 130 ) stat
                        write ( myIO, 320 ) real ( global_stop - global_start, rp ) / real ( clock_count_rate, rp )
                        flush ( myIO )
                        write ( myIO,  *  ) real ( global_stop - global_start, rp ) / real ( clock_count_rate, rp )
            ...
            write ( myIO, '( g0, 2 ( ", ", g0 ) )' ) elements ( k ), time_mean, time_var
            flush ( myIO )
    ...
    110 format ( 'requested size is ', g0, ' elements (', g0,' MB); kind = ', g0 )
    120 format ( 'errmsg = ', g0, '.' )
    130 format ( 'stat = ', g0 )

    200 format ( 'error attempting to open file ', g0 )
    210 format ( 'file unit number from NEWUNIT = ', g0 )
    220 format ( 'iomsg = ', g0, '.' )
    230 format ( 'iostat = ', g0 )

    300 format ( "Date: ", g0, " - ", g0, " - ", g0, ", time: ", g0, ":", g0, ":", g0 )
    310 format ( "Program lauched via ", g0, ".", / )
    320 format ( /, 'total cpu time: ', g0, 's', / )
