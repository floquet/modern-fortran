...
program pi_constant_time
    ...
    !use mpif,                           only : MPI_COMM_WORLD, MPI_INT, MPI_DOUBLE, MPI_SUM, MPI_WTIME
    !use mpi,                            only : MPI_COMM_WORLD, MPI_INT, MPI_DOUBLE, MPI_SUM, MPI_WTIME
    use mpif_riptide,                   only : MPI_COMM_WORLD, MPI_INT, MPI_DOUBLE, MPI_SUM, MPI_WTIME
    ...
    character ( len = 128 ) :: host_name = 'nemo'
        ...
        refine_mesh: do j = 1, 5                            ! larger j = finer mesh
            ...
            if ( rank == 0 ) then               ! rank 0 finalizes and reports
                ...
                io_out = safeopen_writeappend ( 'results/' // trim( host_name ) // '_gnu.csv' )
