    ...
    character ( len = 1 ) :: mark = '?'
        ...
        refine_mesh: do j = 3, 4                            ! larger j = finer mesh
            if ( rank == 0 ) then                           ! rank 0 grabs start time & # of intervals
                ...
                if ( j == 0 ) write ( * , '( "2**32 - 1 = ", g0, / )' ) huge ( rank )
                mark = ' '
                if ( numIntDomain > huge ( rank ) ) mark = '*'
                write ( * , '( g0, " numIntDomain = ", g0, ", j = ", g0 / )' ) mark, numIntDomain, j
            end if
        ...
        100 format ( g0, ': rank = ', g0, ', j = ', g0, ', numIntDomain + rank = ', g0, ' (should be ', g0, ' + ', g0, ')' )
        110 format ( g0, ': rank = ', g0, ', j = ', g0, ', numIntDomain + rank - rank = ', g0, ' (should be ', g0, ')', / )

        200 format ( 'After MPI_BCAST of ', I20, ' as ', g0, ' ierr = ', g0, '.' )
