...
program lucas_idea
    ...
    use mFileHandling,                  only : safeopen_writenew

    implicit none

    external :: MPI_INIT, MPI_BARRIER, MPI_COMM_RANK, MPI_COMM_SIZE, MPI_FINALIZE, MPI_REDUCE
    ...
    integer ( ip ) :: numIntArchive = 0_ip, numInt8Archive = 0_ip
    integer ( ip ) :: j = 0_ip
    ...
    interface
        ...
        io_out_debug = safeopen_writenew ( 'debug_rank_' // trim ( rank_str ) // '.txt' )

        refine_mesh: do j = 0, 3                            ! larger j = finer mesh
            if ( rank == 0 ) then                           ! rank 0 grabs start time & # of intervals
                numInt = 10 ** j * 123456789            ! scale up: 720720 = LCM( Range[16] )
                if ( numInt .lt. 0 ) exit refine_mesh
                numInt8 = numInt
                numIntArchive = numInt
                numInt8Archive = numInt
                if ( j == 0 ) write ( io_out_debug, '( "2**31 - 1 = ", g0, / )' ) huge ( rank )
                mark = ' '
                if ( numInt > huge ( rank ) ) mark = '*'
                ! write ( * , '( g0, " Preparing to MPI_BCAST numInt = ", g0, ", j = ", g0 )' ) mark, numInt, j
                ! write ( * , '( g0, " Preparing to MPI_BCAST numInt8 = ", g0, ", j = ", g0 )' ) mark, numInt8, j
                ! write ( * , * )
            end if
            ...
            write ( io_out_debug, 200 ) 'MPI_INT', 10 ** j * 123456789, numInt, ierr
            numIntp = numInt + rank
            write ( io_out_debug, 100 ) 'MPI_INT', rank, numIntp, numIntArchive, rank
            ...
            write ( io_out_debug, 200 ) 'MPI_INTEGER8', 10 ** j * 123456789, numInt8, ierr
            numIntp8 = numInt8 + rank
            write ( io_out_debug, 100 ) 'MPI_INTEGER8', rank, numIntp8, numInt8Archive, rank
        ...
        200 format ( g0, ' MPI_BCAST sent: ', g0, ', received: ', g0, ', ierr = ', g0, '.' )
