...
module mFileInquire
    ...
    character ( len = * ), parameter :: myModule = 'module mFileInquire'  ! self-identification, error tracking
...
contains
    ...
    subroutine fileInquire ( io_unit )                                                                             ! fileInquire
        ...
        integer                 :: io_status
        !   flags
        logical                 :: fname, fexist, fopen
        !   basics
        integer                 :: inumber
        character ( len = 64 )  :: cname
        !   records
        integer                 :: irecl, inextrec, ipos
        !   access
        character ( len = 64 )  :: caccess, csequential, cdirect, cstream, cform, cformatted, cunformatted, cposition
        !   action
        character ( len = 64 )  :: caction, cread, cwrite, creadwrite
        !   asynchronous
        integer                 :: iid
        logical                 :: fpending
        character ( len = 64 )  :: casynchronous, cencoding
        !   preferences
        character ( len = 64 )  :: cblank, cdelim, cpad, cround, csign
    ...
    subroutine test_stat ( myFile )                                                                                  ! test_stat
        ...
        character ( len = * ) :: myFile
            ...
            if ( status == 0 ) then
                write ( * , fmt = " ( 'Targetfile:',              T30, A25 ) " ) myFile
                write ( * , fmt = " ( 'Device ID:',               T30, I25 ) " ) buffer ( 1 )
                write ( * , fmt = " ( 'Inode number:',            T30, I25 ) " ) buffer ( 2 )
                write ( * , fmt = " ( 'File mode (octal):',       T30, O25 ) " ) buffer ( 3 )
                write ( * , fmt = " ( 'Number of links:',         T30, I25 ) " ) buffer ( 4 )
                write ( * , fmt = " ( 'Owner''s uid:',            T30, I25 ) " ) buffer ( 5 )
                write ( * , fmt = " ( 'Owner''s gid:',            T30, I25 ) " ) buffer ( 6 )
                write ( * , fmt = " ( 'Device where located:',    T30, I25 ) " ) buffer ( 7 )
                write ( * , fmt = " ( 'File size:',               T30, I25 ) " ) buffer ( 8 )
                write ( * , fmt = " ( 'Last access time:',        T30, A25 ) " ) ctime ( buffer ( 9 ) )
                write ( * , fmt = " ( 'Last modification time',   T30, A25 ) " ) ctime ( buffer ( 10 ) )
                write ( * , fmt = " ( 'Last status change time:', T30, A25 ) " ) ctime ( buffer ( 11 ) )
                write ( * , fmt = " ( 'Preferred block size:',    T30, I25 ) " ) buffer ( 12 )
                write ( * , fmt = " ( 'No. of blocks allocated:', T30, I25 ) " ) buffer ( 13 )
            end if
